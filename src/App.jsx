...
useEffect(() => {
 const data = quizData.map(newQuestion => {
      return [{question: newQuestion.question, options: 
          [{answer: newQuestion.correct_answer, isCorrect: true, isSelected: false},
          {answer: newQuestion.incorrect_answers[0], isCorrect: false, isSelected: false}, 
          {answer: newQuestion.incorrect_answers[1], isCorrect: false, isSelected: false}, 
          {answer: newQuestion.incorrect_answers[2], isCorrect: false, isSelected: false}, 
        ]}]
  ...
  //   console.log(answeredQuestion, answeredQuestions)
  // }
  // console.log(questions)
  return (
    <>
      <main className="flex flex-col justify-center items-center h-screen font-inter">
        {startQuiz ? <Quiz questions={questions} selectOption = {handleSelectAnswer} nextQuestion = {handleNextQuestion}/> 
        : <div className="max-w-110 p-8 flex flex-col items-center justify-center gap-4 bg-white rounded-[20px] shadow-card text-center">
          <h1 className="text-[28px] font-bold text-primary">Ready to test your Knowledge</h1>
          <p className="text-sm text-secondary">Flex your brain muscles and see how much you know!</p>
          <button onClick={handleStartQuiz} className="w-[200px] h-[50px] bg-blueAccent text-white cursor-pointer rounded-3xl shadow-btn transition-all duration-200 hover:shadow-option-hover">Start Quiz</button>
        </div>
        }
